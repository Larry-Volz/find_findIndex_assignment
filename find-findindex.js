...
function findUserByUsername(usersArray, username) {
  foundUser = usersArray.find((val, idx, arr) =>{
...
function removeUser(usersArray, username) {
    ...
    targetIndex = usersArray.findIndex((users, idx) => {
     return users["username"] === username;
    }); //returns found index or -1 if none

    if (targetIndex === -1) return;

    return usersArray.splice(targetIndex, 1)[0]; //why the [0] at the end ?!?
   };
